    ...
    this.ext = (mod, FS) => {
      return {
        ...
        async read(fd, raw_dst_ptr, raw_length) {
          ...
          //console.log("WeaveDrive: fd: ", fd, " Read length: ", to_read, " Reading ahead:", to - to_read - stream.node.position)
          ...
          const start = 0 //stream.node.position
          const end = data.length //to
        ...
        readFromCache(stream, dst_ptr, length) {
          ...
          if (stream.lastReadPosition !== stream.node.position) {
            //console.log("WeaveDrive: Invalidating cache for fd: ", stream.fd, " Current pos: ", stream.node.position, " Last read pos: ", stream.lastReadPosition)
