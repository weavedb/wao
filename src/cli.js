...
const magenta = t => `\x1b[35m${t}\x1b[39m`
...
const detectHBEnv = async () => {
  ...
  if (erlPath) {
    const ver = await getVersion(
      "erl",
      '-eval \'{ok, V} = file:read_file(filename:join([code:root_dir(), "releases", erlang:system_info(otp_release), "OTP_VERSION"])), io:fwrite(V), halt().\' -noshell'
    )
...
const create = async () => {
  ...
  console.log(
    `    ${dim("2.")} Vite + wao/web   ${dim("(React SPA with ArConnect)")}`
  )
  console.log()
  ...
  try {
    ...
    await exec(`cd "${resolve(appdir, "dashboard")}" && npm install --silent`)
    ...
    if (hbMode !== "skip") {
      ...
      if (hbMode === "clone") {
        ...
        t.sub(`tag: ${HB_TAG}`)
        await exec(
          `cd "${appdir}" && git clone --depth 1 --branch ${HB_TAG} ${HB_REPO} HyperBEAM`,
        ...
        if (deps.erl && deps.rebar3 && missing.length === 0) {
          ...
          try {
            const envParts = []
            if (deps.gcc) {
              envParts.push(`CC=gcc-${deps.gcc}`, `CXX=g++-${deps.gcc}`)
            }
            if (deps.cmake) {
            ...
            await exec(
              `cd "${appdir}/HyperBEAM" && ${envPrefix} rebar3 as genesis_wasm compile`,
              { maxBuffer: 50 * 1024 * 1024, timeout: 600000 }
            )
          ...
          t.sub(
            "Run 'cd HyperBEAM && rebar3 compile' after installing dependencies"
          )
          t.done()
        }
      }
    ...
    t.summary(() => {
      ...
      if (hbWarnings.length > 0) {
        console.log()
        console.log(
          `  ${yellow("⚠")}  ${bold("Missing HyperBEAM dependencies:")}`
        )
        for (const w of hbWarnings) console.log(`     ${dim("•")} ${w}`)
        console.log(
          `     ${dim("Install these before running HyperBEAM tests")}`
        )
      }
      ...
      console.log(
        `      ${dim("① Unit tests        in-memory AOS (mainnet WASM device)")}`
      )
      if (hbMode !== "skip")
        console.log(
          `      ${dim("② Integration test  HyperBEAM (requires Erlang)")}`
        )
      if (withFrontend)
        console.log(
          `      ${dim(`${hbMode !== "skip" ? "③" : "②"} Frontend tests    vitest + Playwright`)}`
        )
      console.log(
        `    ${dim("Agent cannot complete tasks until all gates pass.")}`
      )
      console.log()
      console.log(`  ${bold("2. Deploy to mainnet")}`)
      console.log()
      console.log(
        `    ${dim("$")} yarn deploy src/counter.lua --wallet .wallet.json`
      )
      console.log()
    })
  } catch (e) {
...
const build = async () => {
  ...
  if (feature) {
    claudeArgs.push(`/build ${feature}`)
  } else if (existsSync("tasks.json") && existsSync("plan.md")) {
