...
- ✅ Most modern Android phones qualify
- ✅ Examples: Samsung Galaxy S6+, Google Pixel (any), OnePlus 3+, Xiaomi devices
- ❌ NOT supported: 32-bit ARM, x86 Android phones
- 📱 For iOS devices, see [iOS guide](/mobile/ios)
:::
...
```bash [Beta-1 (default)]
curl -sL https://hb-mobile.vercel.app | bash
```

```bash [Beta-3]
curl -sL https://hb-mobile.vercel.app | HB_VERSION=v0.9-milestone-3-beta-3 bash
```
...
The script will:
1. ✓ Verify your device is compatible (ARM64)
2. ✓ Install all Termux dependencies
3. ✓ Set up Alpine Linux environment
4. ✓ Compile HyperBEAM in Alpine
5. ✓ Deploy to Termux and compile NIFs
6. ✓ Start HyperBEAM automatically
...
Then compilation messages, followed by:
...
When successful, you'll see HyperBEAM starting with:
```
================================
HyperBEAM Starting
WiFi IP: 192.168.x.x
...
These commands use `v0.9-milestone-3-beta-1`. For beta-3, replace the version in `git checkout`.
:::
...
ls -lh /tmp/hyperbeam-compiled.tar.gz && echo "✓✓✓ ALPINE COMPILATION COMPLETE ✓✓✓" && exit
```

**Wait for "✓✓✓ ALPINE COMPILATION COMPLETE ✓✓✓"** - you'll automatically return to Termux.
...
[Alpine Linux] → Compile Everything → [Archive (.tar.gz)]
                                     ...
                                     Copy to Termux
                                            ↓
[Termux] → Extract → Recompile NIFs → [Running HyperBEAM]
```
...
:::tip
Run the tunnel in a separate Termux session (swipe from left edge → New session) so HyperBEAM keeps running.
:::
