...
If you want to run a mobile HyperBEAM node, use [Android](/mobile/android) instead.
:::
...
3. Or direct link: [alpine-standard-3.23.0-aarch64.iso](https://dl-cdn.alpinelinux.org/alpine/v3.23/releases/aarch64/alpine-standard-3.23.0-aarch64.iso)
...
6. After install completes, power off the VM:

```sh
poweroff
```

7. Long-press the VM and tap **Edit**
8. In **DRIVES** section:
   - Drag **VirtIO Drive** above **USB Drive** (reorder so VirtIO is first)
9. In **Network** section:
   ...
   - Set:
     ...
     - Host Port: `8734`
   - Tap **Save**
10. Tap **Save** to save VM settings
11. Start the VM again and login with your root password
...
```sh [Beta-3]
curl -sL https://hb-mobile.vercel.app/compile.sh | HB_VERSION=v0.9-FINAL bash
```
