...
Mine pending blocks (ArLocal only).
...
Mint AR on ArLocal. Returns the new balance string.

```js
const balance = await ar.mint(addr, amount) // amount defaults to "1.0"
```

### isArConnect

Returns `true` if the wallet is an ArConnect browser wallet.

```js
const isWallet = ar.isArConnect()
```
...
```js
const { res, err, id } = await ar.transfer(amount, to)
```

You can set a jwk to the 3rd parameter as a sender. Otherwise, the sender is `ar.jwk`.

```js
const { res, err, id } = await ar.transfer(amount, to, jwk)
```
...
```js
const { err, items, tx, id } = await ar.bundle(dataitems)
```
`dataitems` are `[ [ data, tags ], [ data, tags ], [ data, tags ] ]`.
```js
const { err, items, tx, id } = await ar.bundle([
...
Sign and post a pre-built transaction. Returns `{ res, err, id }`.

```js
const { res, err, id } = await ar.postTx(tx, jwk)
```
