...
Process an HTTP response to extract the decoded message.

```js
import { result } from "hbsig"

const decoded = await result(response)
```

### extractPubKey

Extract the public key from signed message headers.

```js
import { extractPubKey } from "hbsig"

const pubKey = extractPubKey(signed)
```

### decodeSigInput

Decode the `signature-input` header into components and params.

```js
import { decodeSigInput } from "hbsig"

const { components, params } = decodeSigInput(signatureInput)
```

### signer

Lower-level signer factory.

```js
import { signer } from "hbsig"

const sign = signer({ jwk, url, signPath })
```

## Browser / No-Crypto Export

`hbsig` also exports a `nocrypto` subpath for browser environments, providing a subset without crypto-heavy dependencies.

```js
import { id, hashpath, rsaid, hmacid, extractPubKey, structured_to, httpsig_from } from "hbsig/nocrypto"
```
...
`hbsig` comes with an accompanying device (`hbsig@1.0` / `dev_hbsig`) on HyperBEAM to validate various encoding strategies.

```bash
git clone https://github.com/ArweaveOasis/wao.git && cd wao
...
You can find comprehensive encoding tests [here](https://github.com/ArweaveOasis/wao/tree/master/hbsig/test).
...
To run all tests, you need to install [HyperBEAM](https://hyperbeam.ar.io/run/running-a-hyperbeam-node.html), compile it, add `.wallet.json`, and set [CWD](/api/hyperbeam#environment-variables) in `.env.hyperbeam`.
