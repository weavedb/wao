...
When no `get` is specified (defaults to `{ get: false }`), `out` in the return is the raw `Data` field as text (not JSON-parsed).
...
```js
const { mid, out } = await p.msg("Action", { Tag1: "value1" }, { get: "TagA" })
```
...
This is a quite common pattern during testing. Doing the same with Wander (`aoconnect`) requires an enormous amount of code, especially if it involves async/await `receive()`.
...
Load and eval a Lua source script on the process.

```js
const { err, res, mid } = await p.load({ src, fills })
```
