...
The `AO` constructor accepts the following options. You can also pass a port number directly: `new AO(4000)`.
...
| `authority` | `string` | built-in default | Authority address |
| `ar` | `AR` \| `object` | - | AR instance or config like `{ port: 4000 }` |
| `aoconnect` | `object` | - | Wander (aoconnect) config `{ MU_URL, SU_URL, CU_URL, GATEWAY_URL }` |
| `in_memory` | `boolean` | `false` | Use in-memory ArMem mode |
| `port` | `number` | - | Shortcut: sets Wander URLs + AR port |
| `wao` | `boolean` | auto | Force WAO variant |
| `variant` | `string` | - | `"ao.TN.1"` or `"ao.N.1"` |

```js
const ao = new AO(4000) // shortcut for { port: 4000 }
const ao2 = new AO({ module_type: "sqlite", port: 4000 })
```
...
```js
const { err, res, out, pid, p } = await ao.deploy({ data, tags, src, fills })
```

You can directly pass the Lua script with `src_data` instead of `src`.

```js
const { err, res, out, pid, p } = await ao.deploy({ data, tags, src_data, fills })
```
...
```js
const { err, res, out, pid, p } = await ao.deploy({ boot: true, tags, src_data, fills })
```
...
you can only get the first `Hello, World!`, but not the second `"Hello, " .. name .. "!"` from the Wander (`aoconnect`) `result` function.
...
```js
const { err, res, out } = await ao.dry({ pid, data, act, tags, check, get })
```
...
```js
const { err, pid, p } = await ao.spwn({ module, scheduler, tags, data, boot, auth, memory })
```

## Wander (aoconnect) Functions

The original Wander (aoconnect) functions `message` | `spawn` | `result` | `assign` | `dryrun` | `monitor` | `unmonitor`  are also available.
...
Apply `<FILL>` replacements to a Lua source without eval. `src` is an Arweave txid, or you can pass the script directly with `data`.

```js
const { err, out } = await ao.transform({ src, data, fills })
```
