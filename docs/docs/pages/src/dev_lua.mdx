...
init_with_inline_module_test() ->
    ...
    ?assert(hb_private:has(<<"state">>, Result, #{})).

init_with_module_id_test() ->
    Module = <<"function hello() return 'world' end">>,
    {ok, ModuleID} = hb_cache:write(Module, #{}),
    Base = #{<<"module">> => ModuleID},
    {ok, Result} = dev_lua:init(Base, #{}, #{}),
    ?assert(hb_private:has(<<"state">>, Result, #{})).
...
normalize_with_snapshot_test() ->
    ...
    ?assert(hb_private:has(<<"state">>, Normalized, #{})),
