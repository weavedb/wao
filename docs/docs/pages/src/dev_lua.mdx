...
snapshot_test() ->
    ...
    code:ensure_loaded(dev_lua),
    ?assert(erlang:function_exported(dev_lua, snapshot, 3)).
```
...
normalize_with_snapshot_test() ->
    ...
    code:ensure_loaded(dev_lua),
    ?assert(erlang:function_exported(dev_lua, normalize, 3)).

normalize_without_snapshot_test() ->
    % normalize requires snapshot or state - verify export only
    Exports = dev_lua:module_info(exports),
...
encode_decode_nested_test() ->
    % Nested structures may be decoded as proplists in Lua
    Term = #{
        <<"users">> => [
            #{<<"name">> => <<"Alice">>, <<"age">> => 30},
            #{<<"name">> => <<"Bob">>, <<"age">> => 25}
    ...
    ?assert(maps:is_key(<<"users">>, Decoded)).
```
