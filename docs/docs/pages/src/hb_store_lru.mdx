...
stop_test() ->
    ...
    {ok, Instance} = hb_store_lru:start(StoreOpts),
    ServerPID = maps:get(<<"pid">>, Instance),
...
write_triggers_eviction_test() ->
    ...
    hb_store_lru:write(StoreOpts, <<"key3">>, Binary),  % Triggers eviction
...
list_nested_test() ->
    ...
    Binary = crypto:strong_rand_bytes(200),
    hb_store_lru:write(StoreOpts, <<"sub/inner/key1">>, Binary),
    {ok, Keys} = hb_store_lru:list(StoreOpts, <<"sub">>),
    ?assertEqual([<<"inner">>], Keys),
