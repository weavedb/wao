...
index_resolution_test() ->
    ...
    ?assertEqual(IndexPage, hb_cache:ensure_all_loaded(Result, Opts)).
...
index_not_found_test() ->
    ...
    Result = dev_manifest:index(ManifestMsg, #{}, #{}),
...
route_simple_path_test() ->
    ...
    {ok, Result} = dev_manifest:route(<<"page1">>, ManifestMsg, #{}, Opts),
...
route_nested_path_test() ->
    ...
    {ok, Result} = dev_manifest:route(<<"nested/page">>, ManifestMsg, #{}, Opts),
    ?assert(is_map(Result)).
```
...
manifest_from_body_test() ->
    ...
    JSON = hb_message:convert(ManifestData, <<"json@1.0">>, <<"structured@1.0">>, Opts),
    Base = #{ <<"body">> => JSON },
    {ok, Manifest} = dev_manifest:manifest(Base, #{}, Opts),
...
linkify_id_test() ->
    ...
    Result = dev_manifest:linkify(Input, #{}),
    ?assertMatch({link, <<"test-id-123">>, _}, Result).
