...
commit_with_basic_auth_test() ->
    ...
    ?assert(maps:is_key(<<"http-auth@1.0">>, Commitments)).
...
verify_valid_signature_test() ->
    ...
    Req = #{<<"authorization">> => <<"Basic ", Credentials/binary>>},
    {ok, Signed} = dev_codec_http_auth:commit(Base, Req, #{}),
    % Verify with same credentials
    VerifyReq = Req#{<<"commitments">> => maps:get(<<"commitments">>, Signed)},
    {ok, IsValid} = dev_codec_http_auth:verify(Base, VerifyReq, #{}),
    ?assert(IsValid).

verify_wrong_credentials_test() ->
    Credentials1 = base64:encode(<<"user:password">>),
    Credentials2 = base64:encode(<<"user:wrongpass">>),
    Base = #{<<"data">> => <<"test">>},
    Req1 = #{<<"authorization">> => <<"Basic ", Credentials1/binary>>},
    {ok, Signed} = dev_codec_http_auth:commit(Base, Req1, #{}),
    % Verify with different credentials
    Req2 = #{
        <<"authorization">> => <<"Basic ", Credentials2/binary>>,
        <<"commitments">> => maps:get(<<"commitments">>, Signed)
    },
    {ok, IsValid} = dev_codec_http_auth:verify(Base, Req2, #{}),
    ?assertNot(IsValid).
...
verify_tampered_message_test() ->
    ...
    Req = #{<<"authorization">> => <<"Basic ", Credentials/binary>>},
    {ok, Signed} = dev_codec_http_auth:commit(Base, Req, #{}),
    % Tamper with base message
    TamperedBase = Base#{<<"data">> => <<"modified">>},
    VerifyReq = Req#{<<"commitments">> => maps:get(<<"commitments">>, Signed)},
    {ok, IsValid} = dev_codec_http_auth:verify(TamperedBase, VerifyReq, #{}),
    ?assertNot(IsValid).
