...
**Description:** Apply store defaults to a list of store configuration maps. For each store in the list, applies defaults based on its `store-module` type and recursively processes any nested stores in the `<<"store">>` field.
...
**Test Code:**
```erlang
-module(hb_store_opts_apply_test).
...
**Test Code:**
```erlang
-module(hb_store_opts_module_type_test).
-include_lib("eunit/include/eunit.hrl").

module_type_lmdb_test() ->
    StoreOpt = #{
        <<"name">> => <<"test">>,
        <<"store-module">> => hb_store_lmdb
    },
...
**Test Code:**
```erlang
-module(hb_store_opts_nested_test).
-include_lib("eunit/include/eunit.hrl").
...
full_integration_flow_test() ->
    ...
    UpdatedStoreOpts = hb_store_opts:apply(StoreOpts, StoreDefaults),
    
    [LmdbStore, GatewayStore] = UpdatedStoreOpts,
    ?assertEqual(5000, maps:get(<<"capacity">>, LmdbStore)),
    
    NestedStores = maps:get(<<"store">>, GatewayStore),
    [NestedLmdb] = NestedStores,
