...
**Description:** Start/stop a monitoring process that prints process group statistics every second. Useful for debugging and observing worker activity.
...
start_stop_monitor_test() ->
    ...
    timer:sleep(100),
    ok = hb_persistent:stop_monitor(Monitor),
    timer:sleep(50),
...
find_or_register_leader_test() ->
    ...
    {Status, _GroupName} = hb_persistent:find_or_register(Msg1, Msg2, Opts),
...
notify_waiting_processes_test() ->
    ...
    TestPID = self(),
    spawn(fun() ->
        TestPID ! {resolve, self(), GroupName, Msg2, #{}}
    end),
    timer:sleep(10),
...
start_worker_custom_function_test() ->
    CustomWorker = fun(GroupName, Msg, Opts) ->
        io:format("Custom worker: ~p~n", [GroupName]),
        hb_persistent:default_worker(GroupName, Msg, Opts)
    end,
    Msg1 = #{<<"device">> => test_device()},
    Worker = hb_persistent:start_worker(CustomWorker, Msg1, #{}),
    ?assert(is_pid(Worker)).

persistent_worker_multiple_requests_test() ->
    TestTime = 200,
    Msg1 = #{<<"device">> => test_device()},
    Worker = hb_persistent:start_worker(Msg1, #{static_worker => true}),
    timer:sleep(10),
    ...
    spawn_test_client(Msg1, Msg3),
    
    % Wait for both to complete
    Res1 = receive_test_result(),
    Res2 = receive_test_result(),
    T1 = erlang:system_time(millisecond),
    
    ?assertNotEqual(Res1, Res2),
    ?assert(T1 - T0 >= TestTime * 2).
```
...
forward_work_test() ->
    Msg = #{<<"device">> => test_device()},
    Opts = #{spawn_worker => true, static_worker => true},
    ok = hb_persistent:forward_work(Msg, Opts),
    timer:sleep(100),
    % Worker should be registered
    GroupName = hb_persistent:group(Msg, undefined, Opts),
    ?assertMatch({ok, _}, hb_persistent:find_execution(GroupName, Opts)).
```
...
group_disabled_test() ->
    ...
    Msg2 = #{<<"path">> => <<"execute">>},
    Opts = #{await_inprogress => false},
    ?assertEqual(ungrouped_exec, hb_persistent:group(Msg1, Msg2, Opts)).
...
default_worker_execution_test() ->
    TestTime = 200,
    Msg1 = #{<<"device">> => test_device()},
    
    % Start worker in separate process
    Worker = spawn_link(fun() ->
        GroupName = hb_persistent:group(Msg1, undefined, #{}),
        hb_persistent:default_worker(GroupName, Msg1, #{static_worker => true})
    end),
...
**Description:** Default await function that blocks until receiving result or worker death notification. Used by devices that don't provide custom await logic.
...
default_await_success_test() ->
    Worker = spawn(fun() ->
        receive
            {resolve, Listener, GroupName, Msg2, _} ->
                Listener ! {resolved, self(), GroupName, Msg2, {ok, <<"result">>}}
        end
    end),
    ...
    % Send request in separate process
    spawn(fun() ->
        Worker ! {resolve, self(), GroupName, Msg2, #{}}
    end),
...
default_await_worker_death_test() ->
    ...
    Msg1 = #{<<"device">> => <<"test@1.0">>},
    Msg2 = #{<<"path">> => <<"test">>},
    Result = hb_persistent:default_await(Worker, test_group, Msg1, Msg2, #{}),
    ?assertEqual({error, leader_died}, Result).
```
