...
start_memory64_test() ->
    ...
    ?assert(is_pid(WASM)),
    ?assert(lists:keymember("fac", 1, Exports)),
    hb_beamr:stop(WASM).
```
...
call_indirect_test() ->
    % Indirect call via function table index
    {ok, File} = file:read_file("test/test-calling.wasm"),
    {ok, WASM, _, _} = hb_beamr:start(File),
    {ok, Result} = hb_beamr:call(WASM, 0, [5.0]),  % Index 0
...
call_invalid_args_test() ->
    ...
    Result = hb_beamr:call(WASM, "fac", [<<"invalid">>]),
...
serialize_test() ->
    ...
    % Execute some function to change state
    {ok, _} = hb_beamr:call(WASM, "fac", [5.0]),
    
    {ok, Memory} = hb_beamr:serialize(WASM),
    ?assert(is_binary(Memory)),
    ?assert(byte_size(Memory) > 0),
