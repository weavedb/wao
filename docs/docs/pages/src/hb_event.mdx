...
%% Counter Management
-spec increment(Topic, Message, Opts) -> ok | ignored.
-spec increment(Topic, Message, Opts, Count) -> ok | ignored.
...
**Description:** Increment Prometheus counter for topic and message. Filters out high-frequency topics to avoid performance costs. Spawns event server if needed.
...
increment_basic_test() ->
    ...
    ?assert(Result == ok orelse Result == ignored).
...
counters_structure_test() ->
    ...
    case maps:get(<<"test_topic">>, Counters, undefined) of
        ...
        TopicMap ->
            ?assert(is_map(TopicMap))
    end.

counters_aggregation_test() ->
    % Increment same event multiple times
    Topic = test_aggregation,
    Event = {test_event, 1},
    
    hb_event:increment(Topic, Event, #{}, 5),
    hb_event:increment(Topic, Event, #{}, 3),
...
diff_multiple_events_test() ->
    Fun = fun() ->
        hb_event:increment(test, {event1, 1}, #{}),
        hb_event:increment(test, {event2, 2}, #{}),
        hb_event:increment(test, {event1, 1}, #{}),
        success
    end,
...
diff_no_events_test() ->
    Fun = fun() -> <<"no events">> end,
    {Diff, Result} = hb_event:diff(Fun),
    ?assertEqual(<<"no events">>, Result),
    ?assert(is_map(Diff)).
```
