...
graphql_owner_filter_test() ->
    Req = #{<<"owner">> => <<"wallet-address">>},
    Result = dev_copycat_graphql:graphql(#{}, Req, #{}),
    ?assertMatch({ok, _Count}, Result).

graphql_recipient_filter_test() ->
    Req = #{<<"recipient">> => <<"recipient-address">>},
    Result = dev_copycat_graphql:graphql(#{}, Req, #{}),
    ?assertMatch({ok, _Count}, Result).

graphql_all_transactions_test() ->
    Req = #{<<"all">> => true},
    Result = dev_copycat_graphql:graphql(#{}, Req, #{}),
    ?assertMatch({ok, _Count}, Result).
```
...
**Test Code:**
```erlang
-module(index_graphql_test).
-include_lib("eunit/include/eunit.hrl").

index_graphql_pagination_test() ->
    % Mock setup would be needed for real test
    Query = <<"query($after: String) { transactions(after: $after) { ... } }">>,
    Vars = #{},
    Result = dev_copycat_graphql:index_graphql(0, Query, Vars, undefined, undefined, #{}),
    ?assertMatch({ok, _Total}, Result).
```
