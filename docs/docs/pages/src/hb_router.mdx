...
**Test Code:**
```erlang
-module(hb_router_find_test).
-include_lib("eunit/include/eunit.hrl").

find_basic_test() ->
    % Configure nodes in global options
    Opts = #{
        nodes => #{
            <<"scheduler">> => #{
    ...
    application:set_env(hyperbeam, nodes, maps:get(nodes, Opts)),
    
    Result = hb_router:find(<<"scheduler">>, <<"any-id">>),
    ?assertMatch({ok, <<"https://scheduler.ao.computer">>}, Result).

find_with_address_test() ->
    Opts = #{
        nodes => #{
            <<"compute-unit">> => #{
    ...
    Result1 = hb_router:find(<<"compute-unit">>, <<"task-1">>, <<"primary">>),
    ?assertMatch({ok, <<"https://cu1.ao.computer">>}, Result1),
    
    Result2 = hb_router:find(<<"compute-unit">>, <<"task-1">>, <<"backup">>),
    ?assertMatch({ok, <<"https://cu2.ao.computer">>}, Result2).

find_any_address_test() ->
    Opts = #{
        nodes => #{
            <<"messenger">> => #{
    ...
    Result = hb_router:find(<<"messenger">>, <<"msg-123">>, '_'),
    ?assertMatch({ok, <<"https://mu.ao.computer">>}, Result).

find_not_found_test() ->
    Opts = #{nodes => #{}},
    application:set_env(hyperbeam, nodes, maps:get(nodes, Opts)),
    
    Result = hb_router:find(<<"nonexistent">>, <<"id">>),
    ?assertEqual({error, service_type_not_found}, Result).

find_type_exists_address_not_test() ->
    Opts = #{
        nodes => #{
            <<"service">> => #{
                <<"addr1">> => <<"https://node1.com">>
            }
        }
    },
    application:set_env(hyperbeam, nodes, maps:get(nodes, Opts)),
    
    % Type exists but address doesn't
    Result = hb_router:find(<<"service">>, <<"id">>, <<"nonexistent">>),
    ?assertEqual({error, service_type_not_found}, Result).
```
...
```erlang
application:set_env(hyperbeam, nodes, NodesMap).
```
