...
simple_resolver_test() ->
    ...
    Result = dev_name:resolve(
        <<"hello">>,
        #{},
        #{ <<"load">> => false },
        #{ name_resolvers => [Resolver] }
    ),
    ?assertEqual({ok, <<"world">>}, Result).
...
multiple_resolvers_test() ->
    Resolver1 = #{
        <<"device">> => #{
            <<"lookup">> => fun(_, _, _) -> {error, not_found} end
        }
    },
    Resolver2 = #{
        <<"device">> => #{
    ...
    Result = dev_name:resolve(
        ...
        #{},
        #{ <<"load">> => false },
        #{ name_resolvers => [Resolver1, Resolver2] }
    ),
...
load_from_cache_test() ->
    ...
    {ok, Loaded} = dev_name:resolve(
        <<"mydata">>,
        #{},
        #{ <<"load">> => true },
        #{ name_resolvers => [Resolver] }
    ),
    ?assertEqual(Data, hb_cache:ensure_all_loaded(Loaded, #{})).
```
