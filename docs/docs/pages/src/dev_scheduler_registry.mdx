...
find_with_creation_test() ->
    ...
    Proc = hb_message:commit(#{
        <<"type">> => <<"Process">>,
        <<"scheduler">> => hb_util:human_id(ar_wallet:to_address(hb:wallet())),
        <<"image">> => <<0:256>>
    }, Opts),
    dev_scheduler_registry:start(),
    ID = hb_message:id(Proc, all, Opts),
    Pid = dev_scheduler_registry:find(ID, Proc),
    ?assert(is_pid(Pid)),
...
find_same_id_returns_same_pid_test() ->
    Opts = #{
        store => hb_test_utils:test_store(),
        priv_wallet => Wallet = ar_wallet:new()
    },
    ...
    dev_scheduler_registry:start(),
    ID = hb_message:id(Proc, all, Opts),
    Pid1 = dev_scheduler_registry:find(ID, Proc, Opts),
    Pid2 = dev_scheduler_registry:find(ID, Proc, Opts),
...
get_processes_after_creation_test() ->
    ...
    Proc1 = hb_message:commit(#{
        <<"type">> => <<"Process">>,
        <<"scheduler">> => hb_util:human_id(ar_wallet:to_address(Wallet)),
        <<"image">> => crypto:strong_rand_bytes(32)
    }, Opts),
