...
schedule_message_test() ->
    ...
    {ok, _} = hb_ao:resolve(Process, Msg, #{}),
    
    % Verify scheduled
    {ok, Schedule} = hb_ao:resolve(
        Process,
        #{ <<"path">> => <<"schedule">>, <<"method">> => <<"GET">> },
        #{}
    ),
    ?assert(is_list(Schedule) orelse is_map(Schedule)).
...
compute_dryrun_test() ->
    ...
    ?assertEqual(<<"99">>, hb_ao:get(<<"results/data">>, DryResult, #{})).
...
snapshot_creation_test() ->
    ...
    % Create snapshot
    {ok, Snapshot} = hb_ao:resolve(
        Process,
        #{ <<"path">> => <<"snapshot">> },
        #{}
    ),
    ?assert(maps:is_key(<<"cache-control">>, Snapshot)).
```
...
as_execution_test() ->
    ...
    ?assertEqual(<<"wasm@1.0">>, maps:get(<<"device">>, AsExecution)).
