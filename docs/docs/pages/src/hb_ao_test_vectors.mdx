...
```erlang
-spec run_test() -> ok | skip.
```

**Description:** Command-line hook for running specific tests. Execute via:
```bash
...
```erlang
-spec suite_test_() -> TestSuite.
```

**Description:** Main test suite generator. Runs all tests with different option combinations.
...
```erlang
-spec benchmark_test_() -> BenchmarkSuite.
```

**Description:** Performance benchmark suite for resolution operations.
