...
**Behavior:**
- **With Client:** Sends `{hb_logger, LoggerPID, done, Activity}` when all processes terminate
...
start_with_client_test() ->
    ...
    receive
        {hb_logger, Logger, done, Activity} ->
...
multiple_processes_test() ->
    ...
    receive
        {hb_logger, Logger, done, Activity} ->
            % Should have logs from all 5 workers
            ?assert(length(Activity) >= 10)
    after 5000 ->
        ?assert(false)
    end.
```
...
1. **No Process Monitoring:** `erlang:monitor/2` call is commented out
2. **Console Only:** No file logging support
3. **Memory:** All activity kept in memory
4. **No Filtering:** Cannot filter by log level or type
5. **Simple Format:** Basic console output format
6. **Reverse Order:** Activity stored newest-first
