...
**Description:** Execute a function repeatedly for a duration and count iterations. 3-arity version is designed for parallel execution but requires external coordinator to send initial sync message - not intended for direct calling.
...
benchmark_parallel_exported_test() ->
    ...
    ?assert(erlang:function_exported(hb_test_utils, benchmark, 3)).
```
...
compare_events_exported_test() ->
    ...
    ?assert(erlang:function_exported(hb_test_utils, compare_events, 5)).
```
