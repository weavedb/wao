...
**Usage:**
```erlang
% Start from application supervisor
{ok, Pid} = hb_http_client_sup:start_link([Opts]).

% Typically started by application controller
% in hb_sup.erl or similar top-level supervisor
```
...
start_supervisor_test() ->
    ...
    Children = supervisor:which_children(hb_http_client_sup),
    ?assertMatch([{hb_http_client, _Pid, worker, [hb_http_client]}], Children),
    
    % Cleanup
    exit(Pid, shutdown).

child_restart_test() ->
    {ok, _} = hb_http_client_sup:start_link([#{}]),
...
supervisor_restart_intensity_test() ->
    ...
    % Crash child 6 times rapidly (exceeds 5 in 10s limit)
    lists:foreach(
        fun(_) ->
            [{_, ChildPid, _, _}] = supervisor:which_children(hb_http_client_sup),
            exit(ChildPid, kill),
            timer:sleep(100)
        end,
        lists:seq(1, 6)
    ...
    ?assertEqual(undefined, whereis(hb_http_client_sup)).
```
