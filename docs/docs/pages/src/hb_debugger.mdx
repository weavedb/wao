...
start_manual_test() ->
    % This test requires manual debugger attachment
    % Start debugger and connect within 5 seconds
    spawn(fun() ->
        timer:sleep(5000),
        net_kernel:connect_node('debugger@localhost')
    end),
    
    Seconds = hb_debugger:start(),
...
-export([test_function/0, test_function/1]).
...
start_and_break_no_args_manual_test() ->
    % This test requires manual debugger attachment
    % Will set breakpoint on test_function/0
    hb_debugger:start_and_break(?MODULE, test_function).

start_and_break_with_args_manual_test() ->
    % Will set breakpoint on test_function/1
    hb_debugger:start_and_break(?MODULE, test_function, [<<"arg">>]).

start_and_break_with_scope_manual_test() ->
    % Will interpret all hb_* modules and set breakpoint
    hb_debugger:start_and_break(
        ?MODULE, 
        test_function, 
        [],
        <<"hb_,dev_">>  % Interpret hb_* and dev_* modules
    ).
```
...
profile_function_test() ->
    % This test will stop the node, so skip in normal test runs
    % Uncomment to profile a specific function
    ...
    ?assertEqual(skipped, skipped).

profile_with_complex_work_test() ->
    % Example of profiling complex work
    
    % ComplexWork = fun() ->
    %     Data = lists:duplicate(1000, <<"data">>),
    %     lists:map(fun(X) -> crypto:hash(sha256, X) end, Data)
    % end,
    % hb_debugger:profile_and_stop(ComplexWork).
...
-export([breakpoint_function/0]).

breakpoint_function() ->
    <<"will break here">>.

await_breakpoint_manual_test() ->
    % This test requires manual interaction:
    % 1. Attach debugger
    % 2. Set breakpoint on breakpoint_function/0
    
    spawn(fun() ->
        timer:sleep(5000),
        int:break_in(?MODULE, breakpoint_function, 0)
    end),
    
    Breakpoint = hb_debugger:await_breakpoint(),
    ?assert(is_tuple(Breakpoint)).
```
