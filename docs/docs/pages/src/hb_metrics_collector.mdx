...
**Test Code:**
```erlang
-module(hb_metrics_collector_test).
-include_lib("eunit/include/eunit.hrl").

collect_mf_test() ->
    % Mock callback to capture metrics
    Metrics = [],
    Callback = fun(Metric) ->
    ...
    % Collect metrics
    ok = hb_metrics_collector:collect_mf(default, Callback),
...
**Test Code:**
```erlang
-module(hb_metrics_format_test).
-include_lib("eunit/include/eunit.hrl").

collect_metrics_uptime_test() ->
    Uptime = 120000,  % 120 seconds in milliseconds
    Metrics = hb_metrics_collector:collect_metrics(process_uptime_seconds, Uptime),
    ?assert(is_list(Metrics)),
    % Verify conversion to seconds
    [{Labels, Value}] = Metrics,
    ?assertEqual([], Labels),
    ?assertEqual(120.0, Value).

collect_metrics_load_test() ->
    Load = 2.5,
    Metrics = hb_metrics_collector:collect_metrics(system_load, Load),
    [{Labels, Value}] = Metrics,
    ?assertEqual([], Labels),
    ?assertEqual(2.5, Value).
```
