...
sign_different_key_types_test() ->
    ...
    TXRSA = ar_tx:sign(ar_tx:new(<<>>, 1000000, 0, <<>>), {PrivRSA, PubRSA}),
...
verify_tampered_data_test() ->
    ...
    SignedTX = ar_tx:sign(TX, {Priv, Pub}),
    TamperedTX = SignedTX#tx{data = <<"Tampered data">>},
    ?assertEqual(false, ar_tx:verify(TamperedTX)).
...
verify_same_owner_target_test() ->
    ...
    TX = ar_tx:new(OwnerAddress, 1000000, 1000000, <<>>),
    SignedTX = ar_tx:sign(TX, {Priv, Pub}),
...
json_struct_to_tx_basic_test() ->
    {Priv, Pub} = ar_wallet:new(),
    OrigTX = ar_tx:sign(ar_tx:new(<<>>, 1000000, 0, <<>>), {Priv, Pub}),
    JSONStruct = ar_tx:tx_to_json_struct(OrigTX),
    ParsedTX = ar_tx:json_struct_to_tx(JSONStruct),
    ?assertEqual(OrigTX#tx.id, ParsedTX#tx.id),
    ?assertEqual(OrigTX#tx.owner, ParsedTX#tx.owner),
    ?assertEqual(OrigTX#tx.signature, ParsedTX#tx.signature),
    ?assertEqual(OrigTX#tx.quantity, ParsedTX#tx.quantity),
    ?assertEqual(OrigTX#tx.reward, ParsedTX#tx.reward).

json_struct_to_tx_with_data_test() ->
    {Priv, Pub} = ar_wallet:new(),
    Data = <<"Test data">>,
    OrigTX = ar_tx:sign(
        (ar_tx:new(<<>>, 1000000, 0, <<>>))#tx{
            data = Data,
            data_size = byte_size(Data)
        },
        {Priv, Pub}
    ...
    JSONStruct = ar_tx:tx_to_json_struct(OrigTX),
    ParsedTX = ar_tx:json_struct_to_tx(JSONStruct),
    ?assertEqual(Data, ParsedTX#tx.data),
    ?assertEqual(byte_size(Data), ParsedTX#tx.data_size).

json_struct_to_tx_with_tags_test() ->
    {Priv, Pub} = ar_wallet:new(),
    Tags = [{<<"Content-Type">>, <<"text/plain">>}, {<<"App-Name">>, <<"Test">>}],
    OrigTX = ar_tx:sign(
        (ar_tx:new(<<>>, 1000000, 0, <<>>))#tx{tags = Tags},
        {Priv, Pub}
    ),
    JSONStruct = ar_tx:tx_to_json_struct(OrigTX),
    ParsedTX = ar_tx:json_struct_to_tx(JSONStruct),
    ?assertEqual(Tags, ParsedTX#tx.tags).

json_struct_to_tx_with_target_test() ->
    {Priv, Pub} = ar_wallet:new(),
    Target = crypto:strong_rand_bytes(32),
    OrigTX = ar_tx:sign(
        ar_tx:new(Target, 1000000, 5000000, <<>>),
        {Priv, Pub}
    ),
    JSONStruct = ar_tx:tx_to_json_struct(OrigTX),
    ParsedTX = ar_tx:json_struct_to_tx(JSONStruct),
    ?assertEqual(Target, ParsedTX#tx.target),
    ?assertEqual(5000000, ParsedTX#tx.quantity).
```
...
tx_to_json_struct_basic_test() ->
    {Priv, Pub} = ar_wallet:new(),
    TX = ar_tx:sign(ar_tx:new(<<>>, 1000000, 0, <<>>), {Priv, Pub}),
...
tx_to_json_struct_roundtrip_test() ->
    {Priv, Pub} = ar_wallet:new(),
    OrigTX = ar_tx:sign(ar_tx:new(<<>>, 1000000, 0, <<>>), {Priv, Pub}),
    JSONStruct = ar_tx:tx_to_json_struct(OrigTX),
    ParsedTX = ar_tx:json_struct_to_tx(JSONStruct),
...
tx_to_json_struct_quantity_format_test() ->
    {Priv, Pub} = ar_wallet:new(),
    TX = ar_tx:sign(ar_tx:new(<<>>, 1000000, 5000000, <<>>), {Priv, Pub}),
    JSONStruct = ar_tx:tx_to_json_struct(TX),
...
tx_to_json_struct_denomination_test() ->
    {Priv, Pub} = ar_wallet:new(),
    TX = ar_tx:sign(
        (ar_tx:new(<<>>, 1000000, 0, <<>>))#tx{denomination = 10},
        {Priv, Pub}
    ),
    JSONStruct = ar_tx:tx_to_json_struct(TX),
...
| `format` | `integer()` | Transaction format version (1 or 2) |
...
TX = ar_tx:new(Destination, Fee, Amount, LastTX),
SignedTX = ar_tx:sign(TX, {Priv, Pub}),
true = ar_tx:verify(SignedTX).
