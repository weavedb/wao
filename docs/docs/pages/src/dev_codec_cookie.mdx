...
to_set_cookie_format_test() ->
    ...
    [Line] = SetCookies,
    ?assert(binary:match(Line, <<"session=abc123">>) =/= nomatch),
    ?assert(binary:match(Line, <<"Path=/">>) =/= nomatch),
    ?assert(binary:match(Line, <<"HttpOnly">>) =/= nomatch).
...
to_cookie_format_test() ->
    ...
    ?assert(binary:match(Cookie, <<"key1=val1">>) =/= nomatch),
    ?assert(binary:match(Cookie, <<"key2=val2">>) =/= nomatch).
```
...
commit_and_verify_test() ->
    ...
    {ok, IsValid} = dev_codec_cookie:verify(Base, Signed, #{}),
...
finalize_test() ->
    Base = #{},
    Request = #{},
    {ok, Sequence} = dev_codec_cookie:finalize(Base, Request, #{}),
