...
read_basic_test() ->
    {ok, Opts, [_LocalStore, Wallet, _Address, Node]} = setup_test_env(),
    TestData = #{ <<"key">> => <<"value">> },
    {_WriteRes, Path} = write_to_cache(Node, TestData, Wallet),
    Request = #{
        <<"method">> => <<"GET">>,
        <<"target">> => Path
    },
...
read_with_aos_format_test() ->
    ...
    {ok, Result} = dev_cache:read(#{}, Request, Opts),
    ?assertEqual(<<"application/aos-2">>, maps:get(<<"content-type">>, Result)),
    ?assert(is_binary(maps:get(<<"body">>, Result))),
    % Body should be JSON
    Decoded = hb_json:decode(maps:get(<<"body">>, Result)),
...
write_single_message_test() ->
    ...
    {WriteRes, Path} = write_to_cache(Node, TestData, Wallet),
    ?assertEqual(200, maps:get(<<"status">>, WriteRes)),
...
write_unauthorized_test() ->
    Opts = #{ 
        store => [hb_test_utils:test_store()],
        cache_writers => []  % No authorized writers
    },
    ...
    {error, ErrorMsg} = dev_cache:write(#{}, Request, Opts),
    ?assertEqual(403, maps:get(<<"status">>, ErrorMsg)),
    ?assert(is_binary(maps:get(<<"body">>, ErrorMsg))).
...
link_creation_test() ->
    {ok, Opts, [_LocalStore, Wallet, _Address, Node]} = setup_test_env(),
    % Write original data
    TestData = #{ <<"data">> => <<"original">> },
    {_WriteRes, SourcePath} = write_to_cache(Node, TestData, Wallet),
    % Create link
    DestPath = <<"custom-path-alias">>,
    LinkMsg = #{
        <<"path">> => <<"/~cache@1.0/link">>,
        <<"method">> => <<"POST">>,
        <<"source">> => SourcePath,
    ...
    SignedMsg = hb_message:commit(LinkMsg, Wallet),
    {ok, LinkRes} = hb_http:post(Node, SignedMsg, #{}),
    ?assertEqual(200, maps:get(<<"status">>, LinkRes)),
    % Verify link works
    {ok, ReadData} = hb_cache:read(DestPath, Opts),
    ?assert(hb_message:match(TestData, ReadData, only_present, Opts)).
