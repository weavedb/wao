...
-spec type(Item) -> bundle | list | binary.
...
type_list_test() ->
    Item = #tx{data = [#tx{data = <<"item1">>}]},
    ?assertEqual(list, ar_bundles:type(Item)).

type_bundle_test() ->
    Item = #tx{data = #{<<"1">> => #tx{data = <<"item1">>}}},
    ?assertEqual(bundle, ar_bundles:type(Item)).
```
...
map_from_list_test() ->
    Item1 = #tx{data = <<"item1">>},
    Item2 = #tx{data = <<"item2">>},
    Item = #tx{data = [Item1, Item2]},
    Result = ar_bundles:map(Item),
    ?assert(is_map(Result)),
    ?assertEqual(2, maps:size(Result)),
    ?assertEqual(Item1, maps:get(<<"1">>, Result)),
    ?assertEqual(Item2, maps:get(<<"2">>, Result)).
```
...
member_deep_test() ->
    ...
    ?assertEqual(true, ar_bundles:member(<<"key1">>, Outer)),
    ?assertEqual(true, ar_bundles:member(Inner#tx.id, Outer)),
    ?assertEqual(true, ar_bundles:member(Outer#tx.id, Outer)).
```
...
manifest_simple_bundle_test() ->
    ...
    Manifest = ar_bundles:manifest(Bundle),
