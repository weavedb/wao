...
**Test Code:**
```erlang
-module(write_test_message_test).
...
write_message_test() ->
    ...
    {ok, Msg} = dev_query_test_vectors:write_test_message(Opts),
    ?assert(is_map(Msg)),
    ?assertEqual(<<"Message">>, hb_maps:get(<<"type">>, Msg, Opts)),
...
**Test Code:**
```erlang
-module(write_test_message_recipient_test).
-include_lib("eunit/include/eunit.hrl").
...
write_message_with_recipient_test() ->
    ...
    {ok, Msg} = dev_query_test_vectors:write_test_message_with_recipient(AliceAddress, Opts),
    ?assertEqual(AliceAddress, hb_maps:get(<<"target">>, Msg, Opts)).
```
...
**Test Code:**
```erlang
-module(get_test_blocks_test).
-include_lib("eunit/include/eunit.hrl").

populate_blocks_test() ->
    Opts = #{
        priv_wallet => hb:wallet(),
        store => [hb_test_utils:test_store(hb_store_lmdb)]
    },
    Node = hb_http_server:start_node(Opts),
    dev_query_test_vectors:get_test_blocks(Node),
    % Blocks should now be cached
    ok.
```
