...
basic_execution_test() ->
    ...
    ?assertEqual([120.0], hb_ao:get(<<"results/output">>, Result, #{})).
...
compute_with_imports_test() ->
    ...
    {ok, Result} = hb_ao:resolve(Msg2, <<"compute">>, #{}),
    ?assertEqual([32], hb_ao:get(<<"results/output">>, Result, #{})).
```
...
snapshot_and_restore_test() ->
    ...
    ?assertEqual([4], hb_ao:get(<<"results/output">>, Msg3, #{})),
    {ok, State} = hb_ao:resolve(Msg3, <<"snapshot">>, #{}),
    % Restore from snapshot
    NewMsg = Msg0#{ <<"snapshot">> => State },
    {ok, Restored} = hb_ao:resolve(NewMsg, <<"compute">>, #{}),
    ?assertEqual([4], hb_ao:get(<<"results/output">>, Restored, #{})).
```
...
normalize_with_instance_test() ->
    ...
    ?assertEqual(not_found, hb_ao:get(<<"snapshot">>, Normalized, #{})).
...
normalize_from_snapshot_test() ->
    ...
    % Normalize from snapshot
    MsgWithSnapshot = Msg0#{ <<"snapshot">> => Snapshot },
    {ok, Normalized} = dev_wasm:normalize(MsgWithSnapshot, #{}, #{}),
    Instance = dev_wasm:instance(Normalized, #{}, #{}),
...
imported_function_test() ->
    ...
    {ok, Result} = hb_ao:resolve(ComputeMsg, <<"compute">>, #{}),
    ?assertEqual([32], hb_ao:get(<<"results/output">>, Result, #{})).
```
