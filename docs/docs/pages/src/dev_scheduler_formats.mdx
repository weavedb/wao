...
bundle_empty_test() ->
    ...
    ?assertEqual(ProcID, maps:get(<<"process">>, Result)),
...
aos2_empty_test() ->
    ...
    ?assertEqual(ProcID, maps:get(<<"process">>, PageInfo)),
...
aos2_to_assignment_direct_node_test() ->
    % Can also pass the node directly without wrapper
    Input = #{
        <<"assignment">> => #{
            <<"slot">> => <<"10">>,
            <<"data">> => <<>>
    ...
    Result = dev_scheduler_formats:aos2_to_assignment(Input, #{}),
    ?assertEqual(10, maps:get(<<"slot">>, Result)).
```
