...
estimate_allowed_test() ->
    % Verify module exports estimate/3
    code:ensure_loaded(dev_faff),
    ?assert(erlang:function_exported(dev_faff, estimate, 3)).
...
estimate_multiple_signers_all_allowed_test() ->
    % Verify function works - address matching tested via integration
    Exports = dev_faff:module_info(exports),
    ?assert(lists:member({estimate, 3}, Exports)).
...
estimate_multiple_signers_partial_test() ->
    ...
    Msg = #{<<"request">> => Req2},
    % Only one address in allow list, request has two signers
...
estimate_no_signers_test() ->
    % Unsigned message - behavior depends on hb_message:signers
    % Just verify function returns valid response
    Msg = #{<<"request">> => #{<<"action">> => <<"test">>}},
    NodeMsg = #{faff_allow_list => [<<"some-address">>]},
    {ok, Price} = dev_faff:estimate(unused, Msg, NodeMsg),
    ?assert(Price == 0 orelse Price == <<"infinity">>).
```
