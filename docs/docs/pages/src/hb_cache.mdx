...
ensure_loaded_nested_test() ->
    ...
    Outer = #{<<"outer">> => {link, InnerID, #{store => Store}}},
    {ok, OuterID} = hb_cache:write(Outer, Opts),
    
    Link = {link, OuterID, #{store => Store}},
    Loaded = hb_cache:ensure_loaded(Link, Opts),
    
    % Outer is loaded, inner is still a link
    ?assert(is_map(Loaded)),
    OuterVal = maps:get(<<"outer">>, Loaded),
    ?assertMatch({link, _, _}, OuterVal).
```
...
ensure_all_loaded_test() ->
    ...
    Deep = #{<<"deep">> => <<"value">>},
    {ok, DeepID} = hb_cache:write(Deep, Opts),
    
    Middle = #{<<"middle">> => {link, DeepID, #{store => Store}}},
    {ok, MiddleID} = hb_cache:write(Middle, Opts),
    
    Top = #{<<"top">> => {link, MiddleID, #{store => Store}}},
    
    % Fully load
    Loaded = hb_cache:ensure_all_loaded(Top, Opts),
    
    % All links resolved
    ?assert(is_map(Loaded)),
    ?assert(is_map(maps:get(<<"top">>, Loaded))),
    ?assert(is_map(maps:get(<<"middle">>, maps:get(<<"top">>, Loaded)))),
    ?assertEqual(<<"value">>, 
        maps:get(<<"deep">>, 
            maps:get(<<"middle">>, 
                maps:get(<<"top">>, Loaded)))).
...
ensure_all_loaded_list_test() ->
    ...
    ?assert(is_map(lists:nth(1, Loaded))).
```
...
write_binary_test() ->
    ...
    {ok, ResultPath} = hb_cache:write_binary(Path, Binary, Opts),
    ?assertEqual(Path, ResultPath).
...
list_test() ->
    ...
    % Write some items
    {ok, _} = hb_cache:write(#{<<"a">> => <<"1">>}, Opts),
    {ok, _} = hb_cache:write(#{<<"b">> => <<"2">>}, Opts),
    
    List = hb_cache:list(<<"/">>, Opts),
    ?assert(is_list(List)).
...
list_numbered_test() ->
    ...
    lists:foreach(
        fun(N) ->
            Path = <<BasePath/binary, "/", (integer_to_binary(N))/binary>>,
            hb_cache:write_binary(Path, <<N>>, Opts)
        end,
        [1, 2, 3, 5, 10]
    ),
    
    Numbers = hb_cache:list_numbered(BasePath, Opts),
