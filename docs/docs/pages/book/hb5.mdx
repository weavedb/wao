...
> 📖 Reference: [hb_cache](/src/hb_cache)
...
> 📖 Reference: [hb_cache_control](/src/hb_cache_control)
...
Create a test file `src/test/test_cache.erl`:
...
deduplication_test() ->
    ...
    ?debugFmt("ID1: ~p, ID2: ~p", [ID1, ID2]),
    ?assertEqual(ID1, ID2).  %% Content-addressed!
...
```bash
rebar3 eunit --module=test_cache
```
...
> 📖 Reference: [hb_cache](/src/hb_cache) | [hb_cache_control](/src/hb_cache_control)
...
- [hb_cache Reference](/src/hb_cache) — Cache functions
- [hb_cache_control Reference](/src/hb_cache_control) — Cache control logic
- [hb_store Reference](/src/hb_store) — Storage abstraction
- [Full Reference](/hyperbeam) — All modules

**Related Modules**
- [hb_store](/src/hb_store) — Storage interface
- [hb_store_lmdb](/src/hb_store_lmdb) — LMDB storage backend
- [hb_message](/src/hb_message) — Message creation and signing
- [hb_link](/src/hb_link) — Link structure and resolution

**Concepts**
- [HTTP Cache-Control](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) — HTTP caching semantics (for reference)
- [Content-Addressable Storage](https://en.wikipedia.org/wiki/Content-addressable_storage) — The deduplication principle