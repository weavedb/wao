...
Change `crate-type` and add `rustler` in `Cargo.toml`.
...
Create an nif function in `src/lib.rs`.
...
Create an Erlang NIF device in `HyperBEAM/src/dev_add_nif.erl`.
...
Create an Erlang device in `HyperBEAM/src/dev_add.erl`.
