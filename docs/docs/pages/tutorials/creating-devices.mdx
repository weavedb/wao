...
You can create your own HyperBEAM devices with Erlang, Rust, C++, Elixir, or Gleam, and test them using WAO JS SDK.
...
Then add your device to the `preloaded_devices` list in `HyperBEAM/src/hb_opts.erl`.
...
describe("Hyperbeam Legacynet", function () {
  ...
  it("should query a custom device", async () => {
    const { pid } = await hb.spawn({"execution-device": "foo@1.0"})
